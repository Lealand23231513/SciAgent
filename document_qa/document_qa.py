...
def communicate(path:str, query:str) -> str:
    ...
    chain = load_qa_chain(llm=ChatOpenAI(temperature=0.1, max_tokens=1000, model="gpt-3.5-turbo-0125"), 
                       chain_type="stuff",
                       prompt=prompt)
...
dir_ = Path('Pdf_show/examples')

def fn(question: str, filepath: str) -> str:
    output = communicate(filepath, question)
    return output
